...
def minmax_reduction(model, scores, min_growth=0.1, min_atpm=0.1, eps=1e-3, bigM=1e3, default_score=-1.0,
                     ...
                     ref_reactions=None, ref_score=0.0, solver=None, debug_output=None, time_limit=600):
    ...
    Args:
        ...
        ref_reactions (dict): dictionary of reference reactions and their scores
        ref_score (float): score for reference reactions (default: 0.0)
    ...
    if solver.__class__.__name__ == 'SCIPSolver':
        solver.problem.setParam('limits/time', time_limit)
...
def carve_model(model, reaction_scores, inplace=True, default_score=-1.0, uptake_score=0.0, soft_score=1.0,
                ...
                debug_output=None, time_limit=600):
...
def build_ensemble(model, reaction_scores, size, init_env=None, time_limit=600):
    ...
    for i in range(size):
        ...
        sol = minmax_reduction(model, all_scores, solver=solver, time_limit=time_limit)
